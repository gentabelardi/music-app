        ...
        <div ref={scroller} className="flex flex-col flex flex-col pb-32 h-screen  overflow-y-auto">
          ...
          <div className="flex flex-col my-5 mt-24">
            <div className="flex px-5 items-center">
              <h1 className=" text-white font-bold text-xl">Playlist</h1>
            </div>
            <div className="grid p-5 py-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
              <Link href="/artist/12">
                <motion.a animate={{ opacity: [0, 1], x: [-100, 0] }}
                  transition={{ duration: 0.5 }}
                  className="flex cursor-pointer justify-end relative group col-span-2 flex-col p-3 bg-gradient-to-r from-purple-700 to-pink-500 rounded-md">
                  ...
                  <div className="flex flex-col mt-3 pb-3">
                    <h1 className=" text-white font-bold text-3xl">Like Songs</h1>
                    <span className=" text-white/50 font-medium text-sm">0 liked songs</span>
              ...
              <motion.div animate={{ opacity: [0, 1], x: [-100, 0] }}
                transition={{ duration: 0.5 }}
                className="flex cursor-pointer flex-col items-center justify-center group flex-col p-3 bg-white/5 hover:bg-white/10 rounded-md">
                <div className="flex">
                  <motion.button whileTap={{ scale: 1.2 }}
                    className="border border-white/20 p-3 bg-white/10 rounded-full ">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32"><path fill="none" d="M0 0h24v24H0z" /><path d="M11 11V5h2v6h6v2h-6v6h-2v-6H5v-2z" fill="rgba(255,255,255,1)" /></svg>
                  </motion.button>
                </div>
                <h1 className=" text-white font-bold text-md mt-5">New Playlist</h1>
              </motion.div>
              {data.map(i => {
